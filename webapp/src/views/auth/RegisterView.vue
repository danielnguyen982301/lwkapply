...
function validate(): boolean {
  ...
  fieldErrors.password =
    form.password.length >= 8 ? undefined : "Use at least 8 characters.";
...
async function handleSubmit() {
  ...
  try {
    ...
    formError.value =
      auth.error ?? "Could not create your account. Please try again.";
  } finally {
...
<template>
  <form novalidate class="space-y-5" @submit.prevent="handleSubmit">
    ...
    <p
      v-if="formError"
      role="alert"
      class="rounded-card bg-coral/10 px-3 py-2 text-sm text-coral"
    >
      {{ formError }}
    </p>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div>
        <label for="firstName" class="mb-1 block text-sm font-medium text-ink"
          >First name</label
        >
        <input
          id="firstName"
      ...
      <div>
        <label for="lastName" class="mb-1 block text-sm font-medium text-ink"
          >Last name</label
        >
        <input
          id="lastName"
    ...
    <div>
      ...
      <p v-else id="password-hint" class="mt-1 text-sm text-slate">
